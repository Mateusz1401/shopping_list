...
class NextNavigation extends Component {
  onPress = () => {
    const { listShop, list, setError, hideError } = this.props;

    if (listShop === "" || list.length === 0) {
      setError();
  ...
  render() {
    return (
      <TouchableOpacity onPress={this.onPress} style={styles.container}>
        <Text style={styles.text}>{translate("next")}</Text>
      </TouchableOpacity>
    );
  }
}
...
const mapStateToProps = ({ newList: { listShop }, shop: { list } }) => ({
  listShop,
  list,
});
