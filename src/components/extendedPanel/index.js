...
class ExtendedPanel extends Component {
  ...
  onPress = () => {
    ...
    const { list, addNewShop } = this.state;

    if (shop !== "") {
      if (!check_repeating(list, shop)) {
        addNewShop(shop.trim());
        this.setState({ shop: "" });
  ...
  renderRepeatError = () => (
    <Text style={styles.error}>{translate("repeat_info")}</Text>
  );
  ...
  render() {
    const { extended, extended } = this.state;
