...
class ProductsChoice extends Component {
  ...
  onPress = () => {
    ...
    const { productsList, addNewProduct } = this.state;

    if (product !== "") {
      if (!check_repeating(productsList, product)) {
        addNewProduct({
          name: product.trim(),
          quantity: 1,
          done: false,
  ...
  renderRepeatError = () => (
    <Text style={styles.error}>{translate("repeat_info_product")}</Text>
  );
  ...
  render() {
    const { product, showRepeatError } = this.state;
    ...
    return (
      <Container>
        <View style={styles.view}>
          ...
          {showRepeatError && this.renderRepeatError()}
          {this.props.isLoading ? <Loader /> : this.renderList()}
        </View>
      </Container>
    );
  }
